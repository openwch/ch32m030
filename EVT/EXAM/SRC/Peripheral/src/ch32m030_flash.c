...
#define ACR_LATENCY_Mask           ((uint32_t)0xFFFFFFF8)
...
void FLASH_OptionBytePR(u32* pbuf)
{
    ...
    FLASH_BufLoad((OB_BASE), pbuf[0], pbuf[1]);
    FLASH_BufLoad((OB_BASE+8), pbuf[2], pbuf[3]);
 ...
 *            OB_RST_EN_DT512us - Reset IO enable and  Ignore delay time 512us.
 *            OB_RST_EN_DT128us - Reset IO enable and  Ignore delay time 128us.
 *          OB_RST_PIN - Selects reset pin.
 *            OB_RST_PIN_PA15 - PA15 of reset pin.
 *            OB_RST_PIN_PC0 - PC0 of reset pin.
 *           OB_OV_CFG
 *            OB_OV_CFG_NoEN - No enable over voltage protection
 *            OB_OV_CFG_EN - Enable over voltage protection
 *
 * @return  FLASH Status - The returned value can be: FLASH_BUSY, FLASH_ERROR_PG,
 *        FLASH_ERROR_WRP, FLASH_COMPLETE or FLASH_TIMEOUT.
 */
FLASH_Status FLASH_UserOptionByteConfig(uint16_t OB_STOP, uint16_t OB_STDBY, uint16_t OB_RST, uint16_t OB_RST_PIN, uint16_t OB_OV_CFG)
{
    FLASH_Status status = FLASH_COMPLETE;
    uint8_t UserByte;
    uint32_t buf[4];
    ...
    if((FLASH->OBR & RDPRT_Mask) != (uint32_t)RESET)
    {
        ...
        UserByte = OB_STOP | (uint16_t)(OB_STDBY | (uint16_t)(OB_RST | (uint16_t)(OB_RST_PIN | ( OB_OV_CFG | (uint16_t)0x41))));
 ...
 *
 * @return  The FLASH User Option Bytes values: RST_STOP(Bit1) ,RST_STDBY(Bit2)
 *        ,RST_MODE(Bit3~4) ,RST_PIN_SEL(Bit5) ,OV_CFG(Bit7).
 */
 ...
 * @fn      FLASH_BufLoad
 *
 * @brief   Flash Buffer load(64 bit).
 *
...
{
    if(((Address >= ValidAddrStart) && (Address < ValidAddrEnd)) || ((Address >= OB_BASE) && (Address < (OB_BASE+0x80))))
    {
        FLASH->CTLR |= CR_PAGE_PG;
 ...
 *
 * @brief   Program a specified FLASH page (1page = 128Byte).
 *
...
/*********************************************************************
 * @fn      FLASH_ECCERR_ITCmd
 *
 * @brief   Enables or disables the ECC error interrupts function.
 *
...
/*********************************************************************
 * @fn      FLASH_ECCERR_Recovery_SetErrFlagCmd
 *
 * @brief   Enables or disables  ECC error flag is set after the ECC error recovery.
 *
 * @param   NewState - ENABLE or DISABLE.
 *
 * @return  none
...
/*********************************************************************
 * @fn      FLASH_GetECCFlagStatus
 *
 * @brief   Checks whether the specified FLASH ECC flag is set or not.
 *
 * @param   FLASH_ECC_FLAG - specifies the FLASH ECC flag to check.
 *            FLASH_FLAG_ECC_CKEFail_RECFail - FLASH ECC error check fail
 *          and error recovery fail flag
 *            FLASH_FLAG_ECC_CKEFail_RECSuc - FLASH ECC error check fail
 *          and error recovery success flag
 *            FLASH_FLAG_ECC_CKESuc - FLASH ECC error check success flag
 *            FLASH_FLAG_ECC_CKEFail - FLASH ECC error check fail flag
 *
 * @return  The new state of FLASH_FLAG (SET or RESET).
 */
FlagStatus FLASH_GetECCFlagStatus(uint32_t FLASH_ECC_FLAG)
{
    ...
    if((FLASH->STATR & FLASH_ECC_FLAG) != (uint32_t)RESET)
    {
        bitstatus = SET;
    }
    else
...
/*********************************************************************
 * @fn      FLASH_ClearECCFlag
 *
 * @brief   Clears the FLASH's ECC pending flags.
 *
 * @param   FLASH_ECC_FLAG - specifies the FLASH ECC flags to clear.
 *            FLASH_FLAG_ECC_CKEFail - FLASH ECC error check fail flag.
 *
 * @return  none
 */
