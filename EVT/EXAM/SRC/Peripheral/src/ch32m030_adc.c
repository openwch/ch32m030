...
/* ADC JEXTSEL mask */
#define CTLR2_JEXTSEL_Reset              ((uint32_t)0xFFFF87FF)
...
/* CTLR2 register Mask */
#define CTLR2_CLEAR_Mask                 ((uint32_t)0xFFF0FBFD)
...
/* ADC SMPx mask */
#define SAMPTR1_SMP_Set                  ((uint32_t)0x00000003)
#define SAMPTR2_SMP_Set                  ((uint32_t)0x00000003)
...
#define ADC_IT_Mask                      ((uint32_t)0x0000FFFF)
/*********************************************************************
 * @fn      ADC_DeInit
 *
...
{
    uint32_t itmask = 0, tmp = 0 , tmpmask = 0;

    tmp = ADC_IT & ADC_IT_Mask;
    tmpmask = (ADC_IT & 0x000F0000) >> 16;

    itmask = (uint32_t)(tmp << (16*tmpmask));

    if((ADC_IT & 0xF0000000) != 0)
    {
        if(NewState != DISABLE)
        {
            ADCx->CTLR3 |= itmask;
 ...
 *            ADC_SampleTime_5Cycles5 - Sample time equal to 5.5 cycles.
 *            ADC_SampleTime_11Cycles5 - Sample time equal to 11.5 cycles.
 *            ADC_SampleTime_23Cycles5 - Sample time equal to 23.5 cycles.
 *            ADC_SampleTime_59Cycles5 - Sample time equal to 59.5 cycles.
 *
 * @return  None
 */
void ADC_RegularChannelConfig(ADC_TypeDef *ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
    ...
    if(ADC_Channel > ADC_Channel_15)
    {
        ...
        tmpreg2 = SAMPTR1_SMP_Set << (2 * (ADC_Channel - 16));
        tmpreg1 &= ~tmpreg2;
        tmpreg2 = (uint32_t)ADC_SampleTime << (2 * (ADC_Channel - 16));
        tmpreg1 |= tmpreg2;
        ADCx->SAMPTR1 = tmpreg1;
    }
    else
    {
        if(((*(uint32_t*)0x1FFFF384 & 0x000000F0) == 0) && (ADC_Channel == ADC_Channel_15))
        {
            ADC_SampleTime = ADC_SampleTime_5Cycles5;
        }
        tmpreg1 = ADCx->SAMPTR2;
        tmpreg2 = SAMPTR2_SMP_Set << (2 * ADC_Channel);
        tmpreg1 &= ~tmpreg2;
        tmpreg2 = (uint32_t)ADC_SampleTime << (2 * ADC_Channel);
        tmpreg1 |= tmpreg2;
...
/*********************************************************************
 * @fn      ADC_GetConversionChannelSequence
 *
 * @brief   Returns the last ADCx conversion channel sequence for regular channel.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *
 * @return  The conversion channel sequence.
 */
uint8_t ADC_GetConversionChannelSequence(ADC_TypeDef *ADCx)
{
    return (uint8_t)(ADCx->RDATAR >> 16);
}
 ...
 *          ADC_ExternalTrigInjecConv_T1_CC5 - Timer1 capture compare5 selected.
 ...
 *          ADC_ExternalTrigInjecConv_T1_CC4_CC5 - Timer1 capture compare4 and compare5 selected.
 *          ADC_ExternalTrigInjecConv_PA14_PB6 - PA14 PB6.
 *
 * @return  None
 */
 ...
 *            ADC_FLAG_AWD_OV - Analog watchdog overflow event flag.
 *            ADC_FLAG_AWD_DN - Analog watchdog underflow event flag
 *            ADC_FLAG_MULT_CMP1 - Multi threshold comparison result 3.
 *            ADC_FLAG_MULT_CMP2 - Multi threshold comparison result 2.
 *            ADC_FLAG_MULT_CMP3 - Multi threshold comparison result 1.
 *            ADC_FLAG_AVG_OVI - In the move average function,the conversion value overflow interrupt flag.
 *            ADC_FLAG_AVG_DNI - In the move average function,the conversion value underflow interrupt flag.
 *            ADC_FLAG_AVG_OV - In the move average function,the conversion value overflow flag.
 *            ADC_FLAG_AVG_DN - In the move average function,the conversion value underflow flag.
 *
 * @return  FlagStatus: SET or RESET.
 */
FlagStatus ADC_GetFlagStatus(ADC_TypeDef *ADCx, uint32_t ADC_FLAG)
{
    ...
    flagmask = ADC_FLAG & 0x0FFFFFFF;

    if((ADC_FLAG & 0xF000000) != 0)
    {
        if((ADCx->CTLR3 & flagmask) != (uint8_t)RESET)
        {
            bitstatus = SET;
        }
        else
    ...
    {
        if((ADCx->STATR & flagmask) != (uint8_t)RESET)
        {
            bitstatus = SET;
        }
        else
...
ITStatus ADC_GetITStatus(ADC_TypeDef *ADCx, uint32_t ADC_IT)
{
    ...
    uint32_t itmask = 0, tmp = 0 ,tmp1 = 0 , tmpmask = 0, enablestatus = 0;

    tmp = ADC_IT & ADC_IT_Mask;
    tmp1 = (ADC_IT & 0x00F00000) >> 20;
    tmpmask = (ADC_IT & 0x000F0000) >> 16;
    ...
    if((ADC_IT & 0xF0000000) != 0)
    {
        enablestatus = (ADCx->CTLR3 & itmask);
        if(((ADCx->CTLR3  & (uint32_t)(tmp << tmp1)) != (uint32_t)RESET) && enablestatus)
        {
            bitstatus = SET;
        }
        else
    ...
    {
        enablestatus = (ADCx->CTLR1 & itmask);
        if(((ADCx->STATR  & (uint32_t)(tmp >> tmp1)) != (uint32_t)RESET) && enablestatus)
        {
            bitstatus = SET;
        }
        else
...
{
    uint32_t itmask = 0, tmp = 0 ,tmp1 = 0;
 ...
 * @fn      ADC_MULTCmpCmd
 *
 * @brief   Enables or disables the Multi-threshold comparison function.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *          NewState - ENABLE or DISABLE.
 *
...
/*********************************************************************
 * @fn      ADC_RegularChannelIndicationCmd
 *
 * @brief   Enable ADC regular data channel indication.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *          NewState - ENABLE or DISABLE.
 *
...
/*********************************************************************
 * @fn      ADC_ChannelAdvanceSwCmd
 *
 * @brief   Enable ADC sample channel advance switching.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *          NewState - ENABLE or DISABLE.
 *
...
/*********************************************************************
 * @fn      ADC_RegularChannelPollSaveCmd
 *
 * @brief   Enable the saving function of ADC regular channel data polling.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *          NewState - ENABLE or DISABLE.
 *
...
/*********************************************************************
 * @fn      ADC_CmpareResultXORCmd
 *
 * @brief   Enable ADC comparison result XOR function.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *          NewState - ENABLE or DISABLE.
 *
...
/*********************************************************************
 * @fn      ADC_MovingAverageCmd
 *
 * @brief  Enable ADC moving average function.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *          NewState - ENABLE or DISABLE.
 *
...
/*********************************************************************
 * @fn      ADC_MovingAverageWindowSizeConfig
 *
 * @brief  Set ADC moving average window value
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *          ADC_MoveSize - ADC moving window size.
 *          ADC_MovingWindow_Size_32 - The average value obtained by ADC sampling 32 data points.
 *          ADC_MovingWindow_Size_64 -The average value obtained by ADC sampling 64 data points
 *          ADC_MovingWindow_Size_128 - The average value obtained by ADC sampling 128 data points.
 *          ADC_MovingWindow_Size_256 - The average value obtained by ADC sampling 256 data points
 *          ADC_MovingWindow_Size_512 - The average value obtained by ADC sampling 512 data points
 *          ADC_MovingWindow_Size_1024 - The average value obtained by ADC sampling 1024 data points
 *
 * @return  none
 */
void ADC_MovingAverageWindowSizeConfig(ADC_TypeDef *ADCx, uint32_t ADC_MoveSize)
{
    uint32_t tmpreg1 = 0;
    tmpreg1 = ADCx->CTLR3;
    tmpreg1 &= (uint32_t)0xFF8FFFFF;
    tmpreg1 |= ADC_MoveSize;
    ADCx->CTLR3 = tmpreg1;
}

/*********************************************************************
 * @fn      ADC_AverageValueThresholdsConfig
 *
 * @brief   Configures the high and low thresholds of the analog watchdog.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 ...
 * @return  none
 */
void ADC_AverageValueThresholdsConfig(ADC_TypeDef *ADCx, uint16_t HighThreshold,
                                        uint16_t LowThreshold)
{
    ADCx->AVGTR = (uint32_t)HighThreshold<<16;
    ADCx->AVGTR |= (uint32_t)LowThreshold;
}
...
/*********************************************************************
 * @fn      ADC_GetAverageValue
 *
 * @brief   Returns the last ADCx average value result .
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *
 * @return  ADCx->AVGDR - The Data average value.
 */
uint16_t ADC_GetAverageValue(ADC_TypeDef *ADCx)
{
    return (uint16_t)ADCx->AVGDR;
}

/*********************************************************************
 * @fn      ADC_GetConversionValue2
 *
 * @brief   Returns the last ADCx conversion result data for regular channel 2.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *
 * @return  ADCx->RDATAR2 - The Data conversion value from channel 2.
 */
uint16_t ADC_GetConversionValue2(ADC_TypeDef *ADCx)
{
    return (uint16_t)ADCx->RDATAR2;
}

/*********************************************************************
 * @fn      ADC_GetConversionValue3
 *
 * @brief   Returns the last ADCx conversion result data for regular channel 3.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *
 * @return  ADCx->RDATAR - The Data conversion value from channel 3.
 */
uint16_t ADC_GetConversionValue3(ADC_TypeDef *ADCx)
{
    return (uint16_t)ADCx->RDATAR3;
}

/*********************************************************************
 * @fn      ADC_GetConversionValue4
 *
 * @brief   Returns the last ADCx conversion result data for regular channel 4.
 *
 * @param   ADCx - where x can be 1 to select the ADC peripheral.
 *
 * @return  ADCx->RDATAR - The Data conversion value from channel 4.
 */
uint16_t ADC_GetConversionValue4(ADC_TypeDef *ADCx)
{
    return (uint16_t)ADCx->RDATAR4;
}
