...
static void RxInit(void)
{
    ...
    NVIC_SetPriority(EXTI7_0_IRQn, 0<<6);
    ...
    NVIC_SetPriority(TIM2_IRQn, 1<<6);
...
static void TxInit()
{
    ...
    TIM_OC2Init(SoftUARTHardware.txTIM, &TIM_OCInitstructure);

    TIM_OC2PreloadConfig(SoftUARTHardware.txTIM, TIM_OCPreload_Disable);
    ...
    DMA_InitStruct.DMA_PeripheralBaseAddr = (u32)&SoftUARTHardware.txTIM->CH2CVR;
...
void Init(void)
{
    ...
    RCC_PB2PeriphClockCmd(RCC_PB2Periph_GPIOA | RCC_PB2Periph_TIM1 | RCC_PB2Periph_AFIO | RCC_PB2Periph_GPIOB, ENABLE);
...
void Init(u32 baudrate)
{
    ...
    // Select the peripheral used for the SoftUart
    SoftUARTHardware.rxTIM = TIM2;
    SoftUARTHardware.rxPort = GPIOA;
    SoftUARTHardware.rxPin = GPIO_Pin_7;
    SoftUARTHardware.txTIM = TIM1;
    SoftUARTHardware.txPort = GPIOB;
    SoftUARTHardware.txPin = GPIO_Pin_11;
    SoftUARTHardware.txDMA = DMA1_Channel6;
    // Configure the periph's para
