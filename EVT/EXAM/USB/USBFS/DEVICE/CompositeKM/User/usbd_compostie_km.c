...
volatile uint16_t KB_Scan_Result = 0x3C00;                                      // Keyboard Keys Current Scan Result
volatile uint16_t KB_Scan_Last_Result = 0x3C00;                                 // Keyboard Keys Last Scan Result
...
void KB_Scan_Init( void )
{
    ...
    /* Initialize GPIOA (Pin10-Pin13) for the keyboard scan */
...
void KB_Sleep_Wakeup_Cfg( void )
{
    ...
    EXTI->INTENR |= EXTI_INTENR_MR10 | EXTI_INTENR_MR11 | EXTI_INTENR_MR12 | EXTI_INTENR_MR13;
}
...
void KB_Scan( void )
{
    ...
    if( ( scan_cnt % 10 ) == 0 )
    {
        ...
        if( scan_result == ( GPIO_ReadInputData( GPIOA ) & (GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_12 | GPIO_Pin_13 ) ) )
        {
            KB_Scan_Done = 1;
    ...
    else if( ( scan_cnt % 5 ) == 0 )
    {
        ...
        scan_result = ( GPIO_ReadInputData( GPIOA ) & (GPIO_Pin_10 | GPIO_Pin_11 | GPIO_Pin_12 | GPIO_Pin_13 ) );
    }
}
...
{
    EXTI_ClearFlag( EXTI_Line10 | EXTI_Line11 | EXTI_Line12 | EXTI_Line13 );
    EXTI_ClearFlag( EXTI_Line4 | EXTI_Line5 | EXTI_Line6 | EXTI_Line7 );

    printf( "Sleep\r\n" );
    __WFE( );

    if( EXTI_GetFlagStatus( EXTI_Line10 | EXTI_Line11 | EXTI_Line12 | EXTI_Line13 ) != RESET  )
    {
        EXTI_ClearFlag( EXTI_Line10 | EXTI_Line11 | EXTI_Line12 | EXTI_Line13 );
        USBFS_Send_Resume( );
    }
