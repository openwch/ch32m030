 ...
 * @fn      UART1_CfgInit
 *
 * @brief   UART1 configuration initialization
 *
 * @return  none
 */
void UART1_CfgInit( uint32_t baudrate, uint8_t stopbits, uint8_t parity )
{
    ...
    /* clear te/re */
    USART1->CTLR1 &= ~( UART_CTLR1_TE | UART_CTLR1_RE );
 ...
 * @fn      UART1_ParaInit
 *
 * @brief   UART1 parameters initialization
 ...
 * @fn      UART1_DMAInit
 *
 * @brief   UART1 DMA configuration initialization
 *          type = 0 : USART1_TX
 *          type = 1 : USART1_RX
...
void UART1_DMAInit( uint8_t type, uint8_t *pbuf, uint32_t len )
{
    ...
    if( type == 0x00 )
    {
        ...
        DMA_DeInit( DEF_UART1_TX_DMA_CH );
        ...
        DMA_Init( DEF_UART1_TX_DMA_CH, &DMA_InitStructure );
    ...
    else
    {
        ...
        DMA_DeInit( DEF_UART1_RX_DMA_CH );
        DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)(&USART1->DATAR);
        DMA_InitStructure.DMA_MemoryBaseAddr = (u32)pbuf;
        ...
        DMA_InitStructure.DMA_Priority = DMA_Priority_Medium;
        DMA_InitStructure.DMA_M2M = DMA_M2M_Disable;
        DMA_Init( DEF_UART1_RX_DMA_CH, &DMA_InitStructure );

        DMA_Cmd( DEF_UART1_RX_DMA_CH, ENABLE );
    }
}
 ...
 * @fn      UART1_Init
 *
 * @brief   UART1 total initialization
 *          mode     : See the useage of UART1_ParaInit( mode )
...
void UART1_Init( uint8_t mode, uint32_t baudrate, uint8_t stopbits, uint8_t parity )
{
    ...
    DMA_Cmd( DEF_UART1_RX_DMA_CH, DISABLE );
 ...
 * @fn      UART1_USB_Init
 *
 * @brief   UART1 initialization in usb interrupt
 *
 * @return  none
 */
void UART1_USB_Init( void )
{
    ...
    /* restart usb receive  */
    USBFSD->UEP2_DMA = (uint32_t)(uint8_t *)&UART1_Tx_Buf[ 0 ];
 ...
 *
 * @brief   UART1 data transmission processing
 *
 * @return  none
 */
 ...
 *
 * @brief   UART1 data receiving processing
 *
 * @return  none
 */
