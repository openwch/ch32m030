 ...
 *After successful sending and receiving, PB0 is connected to LED,
...
int main(void)
{
    ...
    while(DMA_GetFlagStatus(DMA1_FLAG_TC6) == RESET) /* Wait until USART1 TX DMA1 Transfer Complete */
    {
    }
    while(DMA_GetFlagStatus(DMA1_FLAG_TC7) == RESET) /* Wait until USART1 RX DMA1 Transfer Complete */
    {
    }
