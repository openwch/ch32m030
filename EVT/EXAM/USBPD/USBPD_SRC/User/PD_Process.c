...
void PD_SRC_Init( )
{
    ...
    EXTEN->EXTEN_CTLR0|=1<<16;
    USBPD->PORT_CC1|=CC_CE;
    USBPD->PORT_CC2|=CC_CE;
    USBPD->PORT_CC1 &= ~CC_PU_Mask;
    USBPD->PORT_CC2 &= ~CC_PU_Mask;
    USBPD->PORT_CC1 = CC_CVS_66 | CC_PU_330;
    USBPD->PORT_CC2 = CC_CVS_66 | CC_PU_330;
}
...
void PD_Init( void )
{
    ...
    EXTEN->EXTEN_KEYR=0x45670123;
    EXTEN->EXTEN_KEYR=0xCDEF89AB;
#if(PD_SEL==PD0)
    EXTEN->EXTEN_CTLR0|=USBPD0_CC_HVT;
#elif(PD_SEL==PD1)
    EXTEN->EXTEN_CTLR0&=~USBPD0_HVT_Mask;
    EXTEN->EXTEN_CTLR0|=USBPD1_CC_HVT;
#endif
...
UINT8 PD_Detect( void )
{
    ...
    if(PD_Ctl.Flag.Bit.Connected)                                       /*Detect disconnection*/
    {
        ...
        if((GPIOA->INDR & PIN_CC1) != (uint32_t)Bit_RESET)
        {
            cmp_cc1 |= bCC_CMP_220;
        }
        if((GPIOA->INDR & PIN_CC2) != (uint32_t)Bit_RESET)
        {
            cmp_cc2 |= bCC_CMP_220;
        }
